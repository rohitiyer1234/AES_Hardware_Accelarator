...
At the current stage, a **dummy AES core** is used for validation of control, AXI, and mode logic. The architecture is explicitly designed so that a **real AES encrypt/decrypt core can be integrated without modifying the AXI or control layers**.

---

## 2. High-Level Architecture
Firmware / CPU (ARM or RISC-V)
|
| AXI4-Lite
|
+-----------------------------+
...
Yep — GitHub is showing it in **one single line** because it’s not inside a **code block**.

✅ Fix: Wrap it like this in your `README.md`:
...
| `0x2C–0x38` | `DATA_OUT[0..3]` | R | Output data block |
...
A dummy AES core is currently used for:
- Validating AXI signaling
- Verifying control FSM behavior
- Testing mode logic correctness

The dummy core:
- Accepts `start`, `enc_dec`, and `data_in`
- Produces deterministic output
- Asserts `done` after fixed latency

This core will be replaced by a **real AES encrypt/decrypt datapath**.

---

### 9.2 AES Encrypt Top (`aes_encrypt_top.v`)
...
- AXI functional verification via Verilog testbench
- Validation of:
  - Register writes/reads
  - START/DONE sequencing
  - Mode-specific behavior
  - Feedback and CTR updates
- AES math correctness deferred to later integration phase

---

## 13. Prospects for Further Development

- Full AES-128/192/256 encrypt/decrypt core
- Key expansion integration
- Round-level pipelining for high throughput
- DMA and interrupt support
- Authenticated modes (GCM/CCM)
- Power and area optimizations
- Secure key storage and side-channel hardening
